...
@section('title', 'Bank Seized Cars for Sale — Quality Repossessed Vehicles in Burlington, VT')

@section('content')

{{-- Hero Carousel --}}
<section class="relative bg-automotive-900 overflow-hidden">
    <div id="hero-carousel" class="relative h-[70vh] min-h-[500px] lg:min-h-[600px]">
        ...
        <div class="hero-slide absolute inset-0 transition-opacity duration-700 ease-in-out" data-active="{{ $index === 0 ? 'true' : 'false' }}" style="opacity: {{ $index === 0 ? 1 : 0 }}; z-index: {{ $index === 0 ? 10 : 0 }};">
            <div class="absolute inset-0">
                ...
                <div class="absolute inset-0 bg-gradient-to-r from-automotive-900/95 via-automotive-900/70 to-automotive-900/30"></div>
            </div>
            <div class="relative z-10 h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center">
                <div class="max-w-2xl">
                    ...
                    <p class="text-lg sm:text-xl text-automotive-200 mb-8 max-w-xl leading-relaxed">{{ $slide->subtitle }}</p>
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ $slide->cta_secondary === 'Find a Part' ? route('shop') : '#' }}" class="inline-flex items-center px-6 py-3.5 border-2 border-white/20 hover:border-white/40 text-white font-semibold rounded-xl transition-all duration-300 hover:bg-white/5">
                            {{ $slide->cta_secondary }}
                        </a>
                    </div>
                </div>
        ...
        <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
            @foreach($slides as $index => $slide)
            <button class="hero-dot w-2.5 h-2.5 rounded-full transition-all duration-300" data-index="{{ $index }}" style="background: {{ $index === 0 ? '#1e40af' : 'rgba(255,255,255,0.4)' }}; {{ $index === 0 ? 'width: 2rem;' : '' }}" aria-label="Slide {{ $index + 1 }}"></button>
            @endforeach
        </div>
...
{{-- Trust Badges --}}
<section class="bg-white border-b border-automotive-100 py-8 lg:py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
            <div class="flex flex-col items-center text-center p-4 lg:p-5 bg-automotive-50 rounded-2xl border border-automotive-100 hover:border-safety/20 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <h4 class="text-sm lg:text-base font-bold text-automotive-900 mb-0.5">Verified Vehicles</h4>
                <p class="text-[11px] lg:text-xs text-automotive-500">Full inspection on every car</p>
            </div>
            <div class="flex flex-col items-center text-center p-4 lg:p-5 bg-automotive-50 rounded-2xl border border-automotive-100 hover:border-safety/20 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <h4 class="text-sm lg:text-base font-bold text-automotive-900 mb-0.5">Transparent History</h4>
                <p class="text-[11px] lg:text-xs text-automotive-500">Clean title documentation</p>
            </div>
            <div class="flex flex-col items-center text-center p-4 lg:p-5 bg-automotive-50 rounded-2xl border border-automotive-100 hover:border-safety/20 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <h4 class="text-sm lg:text-base font-bold text-automotive-900 mb-0.5">Fair Pricing</h4>
                <p class="text-[11px] lg:text-xs text-automotive-500">No hidden fees</p>
            </div>
            <div class="flex flex-col items-center text-center p-4 lg:p-5 bg-automotive-50 rounded-2xl border border-automotive-100 hover:border-safety/20 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <h4 class="text-sm lg:text-base font-bold text-automotive-900 mb-0.5">Expert Support</h4>
                <p class="text-[11px] lg:text-xs text-automotive-500">We help you find the right car</p>
            </div>
        </div>
    </div>
</section>
...
<section class="py-16 lg:py-20 bg-automotive-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10 lg:mb-14">
            <span class="text-safety font-semibold text-sm uppercase tracking-widest">Categories</span>
            <h2 class="text-3xl lg:text-4xl font-bold text-automotive-900 mt-2">Shop by Category</h2>
            <p class="text-automotive-500 mt-3 max-w-2xl mx-auto">Browse our extensive range of automotive components — over 50,000 parts in stock</p>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 lg:gap-5">
            @foreach($categories as $i => $cat)
            <a href="{{ route('shop') }}?category={{ $cat->slug }}" class="group relative bg-white rounded-xl border border-automotive-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 {{ $i >= 6 ? 'hidden lg:block' : '' }}">
...
{{-- Search Bar --}}
<section class="bg-gradient-to-b from-automotive-900 to-automotive-800 py-6 lg:py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <form action="{{ route('shop') }}" method="GET" class="relative" autocomplete="off">
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="e.g. Toyota Camry, Ford F-150, BMW 3 Series..."
                       class="w-full pl-5 pr-14 py-3.5 bg-white rounded-xl text-sm lg:text-base text-automotive-900 placeholder-automotive-400 focus:outline-none focus:ring-2 focus:ring-safety shadow-xl" id="search-hero">
            ...
            <div id="suggest-hero" class="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border border-automotive-100 overflow-hidden hidden z-50 text-left"></div>
        </form>
    </div>
</section>
...
<section class="py-10 lg:py-14 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
            <div>
                <span class="text-cta font-semibold text-sm uppercase tracking-widest">Fresh Inventory</span>
                <h2 class="text-3xl lg:text-4xl font-bold text-automotive-900 mt-2">Latest Vehicles</h2>
                <p class="text-automotive-500 mt-2">Recently added to our inventory — verified and ready for sale</p>
            </div>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            ...
            <div class="group bg-white rounded-xl border border-automotive-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer" onclick="window.location='{{ route('product.show', $product->slug) }}'">
                ...
                <div class="p-3">
                    <p class="text-[10px] text-automotive-400 font-medium mb-1 truncate">{{ $product->compatibility }}</p>
                    <h3 class="font-semibold text-automotive-900 text-xs leading-snug mb-1 line-clamp-2">{{ $product->name }}</h3>
...
<section class="py-16 lg:py-20 bg-automotive-900 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 lg:mb-16">
            <span class="text-safety font-semibold text-sm uppercase tracking-widest">Why Choose Us</span>
            <h2 class="text-3xl lg:text-4xl font-bold text-white mt-2">Buy with Confidence</h2>
            <p class="text-automotive-300 mt-3 max-w-2xl mx-auto">We are committed to providing quality repossessed vehicles and exceptional service to our customers</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-12">
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 lg:p-8 text-center hover:bg-white/10 transition-all duration-300 group">
                ...
                <h3 class="text-white font-semibold mb-2">Free Vehicle Inspection</h3>
                <p class="text-automotive-400 text-sm leading-relaxed">Every vehicle undergoes a comprehensive inspection before listing.</p>
            </div>
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 lg:p-8 text-center hover:bg-white/10 transition-all duration-300 group">
                <div class="w-14 h-14 mx-auto mb-5 bg-safety/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <svg class="w-7 h-7 text-safety" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                </div>
                <h3 class="text-white font-semibold mb-2">Transparent History</h3>
                <p class="text-automotive-400 text-sm leading-relaxed">Full vehicle history reports and clean title documentation provided.</p>
            </div>
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 lg:p-8 text-center hover:bg-white/10 transition-all duration-300 group">
                <div class="w-14 h-14 mx-auto mb-5 bg-safety/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                ...
                <h3 class="text-white font-semibold mb-2">Competitive Pricing</h3>
                <p class="text-automotive-400 text-sm leading-relaxed">Bank-seized vehicles at fraction of market value. No dealer markups.</p>
            </div>
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 lg:p-8 text-center hover:bg-white/10 transition-all duration-300 group">
                <div class="w-14 h-14 mx-auto mb-5 bg-safety/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                ...
                <h3 class="text-white font-semibold mb-2">Expert Guidance</h3>
                <p class="text-automotive-400 text-sm leading-relaxed">Our team helps you find the right vehicle for your needs and budget.</p>
            </div>
        </div>
...
<section class="py-16 lg:py-20 bg-automotive-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
            <div>
                <span class="text-safety font-semibold text-sm uppercase tracking-widest">Featured Vehicles</span>
                <h2 class="text-3xl lg:text-4xl font-bold text-automotive-900 mt-2">Top Picks</h2>
                <p class="text-automotive-500 mt-2">Handpicked vehicles from our current inventory</p>
            </div>
...
<section class="py-16 lg:py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10 lg:mb-14">
            <span class="text-safety font-semibold text-sm uppercase tracking-widest">Makes We Carry</span>
            <h2 class="text-3xl lg:text-4xl font-bold text-automotive-900 mt-2">Popular Car Brands Available</h2>
            <p class="text-automotive-500 mt-3 max-w-2xl mx-auto">We carry seized vehicles from all major manufacturers</p>
        </div>
        <div class="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-5 gap-4 lg:gap-5">
            @foreach($brands as $brand)
            <a href="{{ route('shop') }}?make={{ urlencode($brand->name) }}" class="group bg-white rounded-xl border border-automotive-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                <div class="aspect-square bg-automotive-50 flex items-center justify-center p-6">
                    <img src="{{ $brand->logo }}" alt="{{ $brand->name }}" class="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500" loading="lazy" onerror="this.parentElement.innerHTML='<svg class=\'w-10 h-10 text-automotive-300\' fill=\'none\' stroke=\'currentColor\' viewBox=\'0 0 24 24\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4\'/></svg>'">
                </div>
                <div class="p-3 text-center">
                    <h3 class="font-semibold text-automotive-900 text-sm">{{ $brand->name }}</h3>
                </div>
            </a>
            @endforeach
        </div>
...
<section class="py-16 lg:py-20 bg-automotive-50 overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10 lg:mb-14">
            <span class="text-safety font-semibold text-sm uppercase tracking-widest">Testimonials</span>
            <h2 class="text-3xl lg:text-4xl font-bold text-automotive-900 mt-2">What Our Buyers Say</h2>
            <p class="text-automotive-500 mt-3 max-w-2xl mx-auto">Real reviews from verified customers in Vermont and surrounding areas</p>
        </div>
    </div>
    <div class="relative">
        <div id="testimonials-track" class="flex gap-6" style="animation: scrollTestimonials 40s linear infinite; width: max-content;">
            @for($r = 0; $r < 3; $r++)
            @foreach($testimonials as $testimonial)
            <div class="bg-white rounded-xl border border-automotive-100 p-6 w-80 lg:w-96 flex-shrink-0 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center gap-1 mb-3">
                    @for($i = 0; $i < 5; $i++)
                    <svg class="w-4 h-4 {{ $i < $testimonial->rating ? 'text-yellow-400' : 'text-automotive-200' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                    @endfor
                </div>
                <p class="text-automotive-600 text-sm leading-relaxed mb-4 italic line-clamp-4">"{{ $testimonial->text }}"</p>
                <div class="flex items-center gap-3">
                    <img src="{{ $testimonial->avatar }}" alt="{{ $testimonial->name }}" class="w-9 h-9 rounded-full object-cover" loading="lazy">
                    <div>
                        <p class="font-semibold text-automotive-900 text-sm">{{ $testimonial->name }}</p>
                        <p class="text-automotive-400 text-xs">{{ $testimonial->location }}</p>
                    </div>
                </div>
            </div>
            @endforeach
...
<section class="py-16 lg:py-20 bg-automotive-900 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="text-automotive-300 mb-8 max-w-xl mx-auto">Browse our inventory of bank-repossessed vehicles or contact us for assistance.</p>
        <div class="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <a href="https://wa.me/12174811401" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center px-6 py-3.5 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl transition-all duration-300 shadow-lg shadow-green-500/25 hover:shadow-green-500/40 hover:-translate-y-0.5">
...
(function() {
    ...
    function goTo(index) {
        ...
        dots.forEach((d, i) => {
            d.style.background = i === index ? '#1e40af' : 'rgba(255,255,255,0.4)';
