    ...
    <div id="chatbot-panel" class="chatbot-panel">
        <div class="chatbot-header">
            <div class="chatbot-header-left">
                ...
                <div>
                    <p class="chatbot-header-title">Car Finder AI</p>
                    <p class="chatbot-header-status">Online — Ask me anything</p>
                </div>
            </div>
        ...
        <div id="chatbot-messages" class="chatbot-messages">
            <div class="flex items-start gap-2.5 chatbot-msg bot">
                ...
                <div class="chatbot-msg-bubble bot">
                    <p class="text-sm text-automotive-800">🔧 Hi! Describe the part you need. Example: <span class="text-safety font-medium">"brake discs for Golf 7"</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="chatbot-input-area">
            <form id="chatbot-form" onsubmit="sendChatbotMessage(event)" class="flex gap-2">
                <input id="chatbot-input" type="text" autocomplete="off" placeholder="Describe the part you need..."
                       class="chatbot-input">
...
function addChatMessage(text, isUser) {
    ...
    if (isUser) {
        ...
        div.innerHTML = `
            ...
            <div class="chatbot-msg-bubble bot"><p class="text-sm text-automotive-800">${text}</p></div>`;
    }
...
function showTyping() {
    ...
    div.innerHTML = `
        ...
        <div class="bg-white rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm border border-automotive-100 flex items-center gap-1.5">
            <span class="typing-dot w-2 h-2 bg-automotive-400 rounded-full inline-block"></span>
            <span class="typing-dot w-2 h-2 bg-automotive-400 rounded-full inline-block"></span>
            <span class="typing-dot w-2 h-2 bg-automotive-400 rounded-full inline-block"></span>
        </div>`;
    container.appendChild(div);
...
function showProducts(products) {
    ...
    let html = '<div class="px-3 py-2 text-[11px] font-semibold text-automotive-400 uppercase tracking-wider bg-automotive-50">Matching products</div>';
    products.forEach(p => {
        html += `<a href="${p.url}" target="_blank" class="flex items-center gap-3 px-3 py-2 hover:bg-automotive-50 transition-colors chatbot-product-card border-b border-automotive-50 last:border-0">
            <div class="w-10 h-10 bg-automotive-100 rounded-lg flex-shrink-0 overflow-hidden">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-automotive-900 truncate">${p.name}</p>
                <p class="text-xs text-automotive-400">${p.brand ? p.brand + ' · ' : ''}${p.price}</p>
            </div>
            <svg class="w-4 h-4 text-automotive-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        </a>`;
    });
