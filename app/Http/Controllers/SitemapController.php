...
class SitemapController extends Controller
{
    public function index()
    {
        ...
        $content .= $this->url(route('home'), '1.0', 'daily', now());
        $content .= $this->url(route('shop'), '0.9', 'daily', now());
        $content .= $this->url(route('categories.all'), '0.8', 'weekly', now());
        $content .= $this->url(route('about'), '0.6', 'monthly', now());
        $content .= $this->url(route('contact'), '0.6', 'monthly', now());
        $content .= $this->url(route('delivery'), '0.5', 'monthly', now());
        $content .= $this->url(route('returns'), '0.5', 'monthly', now());
        $content .= $this->url(route('privacy'), '0.3', 'yearly', now());
        $content .= $this->url(route('terms'), '0.3', 'yearly', now());
        $content .= $this->url(route('warranty'), '0.5', 'monthly', now());
